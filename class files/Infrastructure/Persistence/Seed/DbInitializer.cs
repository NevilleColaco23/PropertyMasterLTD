...
static class DbInitializer
{
    ...
    private static void SeedDefaultUser(IServiceProvider services, UserSeedSettings settings)
    {
        ...
        using (var userManager = services.GetRequiredService<UserManager<ApplicationUserIdentity>>())
        {
            if (!userManager.Users.Any(u => u.UserName == settings.DefaultUsername))
            {
                var defaultUser = new ApplicationUserIdentity { UserName = settings.DefaultUsername, Email = settings.DefaultEmail };
